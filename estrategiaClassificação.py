...
def classificação(base_path, ano_inicio = '1/3/2022', janela = False, tamanho_janela = 22, tipo = 'classificação'):
   ...
   # Dividir os dados em treinamento e teste
    ...
    if(janela == False):
        ...
        if tipo == 'baseline':
            num_ones = sum(1 for val in y_test if val == 1)
            num_zeros = sum(1 for val in y_test if val == 0)
            ...
            for _ in range(base.shape[0] - indice_data_inicio + y.index[0]):
                previsao.extend([num])
        else:
